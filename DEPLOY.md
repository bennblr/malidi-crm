...
**ВАЖНО:** Миграции базы данных применяются автоматически при первом запросе к API после деплоя.

**Автоматическое применение миграций:**
- Миграции применяются автоматически при первом запросе к `/api/cards` или другим API endpoints
- Это происходит в фоновом режиме и не блокирует работу приложения
- Логи миграций можно увидеть в логах Render.com
...
**Build Command (рекомендуется):**
```bash
npm install && npm run prisma:generate && npm run build
```

**Примечание:** 
- `prisma migrate deploy` применяет все ожидающие миграции без создания новых (для продакшена)
- Миграции также применяются автоматически при старте приложения через `src/lib/migrate-db.ts`
